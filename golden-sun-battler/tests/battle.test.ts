...
import { QueuedActionPlanner, SimpleAiPlanner } from '../src/planners.js';
...
describe('Golden Sun battle engine', () => {
  ...
  it('steps exactly one round when using nextRound with queued planners', () => {
    const engine = buildEngine('step-seed');
    const firstSnapshot = engine.snapshot;
    const planners: Record<string, ActionPlanner> = {};
    const enemyPlanners: Record<string, ActionPlanner> = {};

    firstSnapshot.parties.players.forEach((player) => {
      const planner = new QueuedActionPlanner(player.id);
      const targetId = firstSnapshot.parties.enemies[0]?.id;
      planner.enqueue({ actorId: player.id, action: player.actions[0], target: targetId });
      planners[player.id] = planner;
    });

    firstSnapshot.parties.enemies.forEach((enemy) => {
      const planner = new SimpleAiPlanner();
      enemyPlanners[enemy.id] = planner;
    });

    const combined = { ...planners, ...enemyPlanners };
    const afterOneRound = engine.nextRound(combined);
    expect(afterOneRound.round).toBe(1);
    expect(afterOneRound.log.length).toBeGreaterThan(0);

    // queued actions have been consumed
    Object.values(planners).forEach((planner) => {
      expect((planner as QueuedActionPlanner).pendingCount()).toBe(0);
    });

    expect(() => engine.nextRound(combined)).toThrowError(/No queued action/);
  });
});

describe('Planner helpers', () => {
  it('SimpleAiPlanner prioritises healing when allies are hurt', () => {
    const heal: PsynergyAction = {
      id: 'heal',
      name: 'Heal',
      category: 'psynergy',
      element: 'venus',
    ...
    const attack: AttackAction = { id: 'strike', name: 'Strike', category: 'attack', target: { type: 'single', id: '' } };
    ...
    const woundedAlly: Combatant = {
      ...
      id: 'ally',
      name: 'Ally',
      stats: { ...caster.stats, hp: 40 }
    };
    const context: PlanningContext = {
      actor: caster,
      allies: [caster, woundedAlly],
      enemies: [],
      round: 1,
      history: [],
      rng: () => 0.5
    ...
    const planner = new SimpleAiPlanner({ healThreshold: 0.5 });
    const command = planner.chooseAction(context);
    expect(command.action.id).toBe('heal');
    expect(command.target).toBe('ally');
  });

  it('QueuedActionPlanner throws when no action is queued', () => {
    const planner = new QueuedActionPlanner('isaac');
    ...
    expect(() => planner.chooseAction(context)).toThrowError('No queued action for isaac');
  });
});
